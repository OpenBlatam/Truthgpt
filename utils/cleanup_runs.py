...
def cleanup_old_runs(runs_dir: str = "runs", days: int = 30, dry_run: bool = True) -> None:
    ...
    if days == 0:
        cutoff_date += timedelta(seconds=2)
    removed = []
    total_size = 0.0
    ...
    if dry_run:
        print(f"\n[INFO] Dry run: Would remove {len(removed)} run(s) ({total_size:.1f} MB total)")
        print("   Use --execute to actually remove them")
    else:
        print(f"\n[OK] Removed {len(removed)} run(s) ({total_size:.1f} MB total)")


def cleanup_checkpoints(runs_dir: str = "runs", keep: int = 3, dry_run: bool = True) -> None:
    ...
    if dry_run:
        print(f"\n[INFO] Dry run: Would remove {total_removed} checkpoint(s) ({total_size:.1f} MB total)")
        print("   Use --execute to actually remove them")
    else:
        print(f"\n[OK] Removed {total_removed} checkpoint(s) ({total_size:.1f} MB total)")
...
def main() -> None:
    ...
    if args.old_runs:
        print(f"[CLEANUP] Cleaning runs older than {args.days} days...")
        cleanup_old_runs(args.runs_dir, args.days, dry_run)

    if args.checkpoints:
        print(f"[CLEANUP] Cleaning checkpoints (keeping last {args.keep_checkpoints})...")
    ...
    if not args.old_runs and not args.checkpoints:
        ...
        print(f"[CLEANUP] Cleaning runs older than {args.days} days...")
        cleanup_old_runs(args.runs_dir, args.days, dry_run)
        print(f"\n[CLEANUP] Cleaning checkpoints (keeping last {args.keep_checkpoints})...")
        cleanup_checkpoints(args.runs_dir, args.keep_checkpoints, dry_run)
