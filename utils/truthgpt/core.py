...
def create_truthgpt_config(**kwargs) -> TruthGPTConfig:
    """Create TruthGPT configuration with flexible argument alias handling."""
    if "device_type" in kwargs and "device" not in kwargs:
        kwargs["device"] = kwargs.pop("device_type")
    if "precision_type" in kwargs and "precision" not in kwargs:
        kwargs["precision"] = kwargs.pop("precision_type")

    if "device" in kwargs and isinstance(kwargs["device"], str):
        try:
            kwargs["device"] = DeviceType(kwargs["device"].lower())
        except ValueError:
            pass
    if "precision" in kwargs and isinstance(kwargs["precision"], str):
        try:
            kwargs["precision"] = PrecisionType(kwargs["precision"].lower())
        except ValueError:
            pass
    if "optimization_level" in kwargs and isinstance(kwargs["optimization_level"], str):
        try:
            kwargs["optimization_level"] = OptimizationLevel(kwargs["optimization_level"].lower())
        except ValueError:
            pass
