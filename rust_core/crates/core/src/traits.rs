//! Trait Abstractions for TruthGPT
//!
//! Generic trait boundaries that downstream crates implement. These traits
//! enable polymorphism across cache backends, compression algorithms, and
//! quantization strategies without introducing circular dependencies.
...
where
    ...
    V: Clone + Debug,
{
    /// Retrieve a reference to the cached value, if present.
...
mod tests {
    ...
    /// Smoke test: verify the trait is object-safe for `Compress`.
    #[test]
    fn test_compress_trait_object_safety() {
        // This test exists purely to verify compilation of dyn Compress.
        fn _accepts_boxed(_c: Box<dyn Compress>) {}
    }

    /// Smoke test: verify `Cache` is object-safe with concrete types.
    #[test]
