...
try:
    from utils.quantum.common import initialize_quantum_state, normalize_state, apply_single_qubit_gate, apply_cnot
    from utils.quantum.revolutionary_quantum_deep_learning_system import RevolutionaryQuantumNeuralLayer, RevolutionaryQuantumConfig
except ImportError:
...
def test_quantum_utilities():
    ...
    # Should be (|00> + |01>) / sqrt(2) -> index 0 and 1
    assert np.isclose(new_state[0], 1/np.sqrt(2))
    assert np.isclose(new_state[1], 1/np.sqrt(2))
    assert np.isclose(new_state[2], 0)
