...
AgentRegistry = importlib.import_module('agents.unified_agent_registry').AgentRegistry
EnterpriseCache = importlib.import_module('utils.enterprise_cache').EnterpriseCache
CacheStrategy = importlib.import_module('utils.enterprise_cache').CacheStrategy
RewardFunctions = importlib.import_module('utils.reward_functions').RewardFunctions
MixtureOfExperts = importlib.import_module('utils.enhanced_mlp').MixtureOfExperts
compute_reward_function = importlib.import_module('utils.enhanced_grpo').compute_reward_function
EnhancedGRPOArgs = importlib.import_module('utils.enhanced_grpo').EnhancedGRPOArgs
ReplayBuffer = importlib.import_module('utils.experience_buffer').ReplayBuffer
...
""")
    ...
    def cmd_email_sequence(self, product: str, persona: str = "ceo_b2b") -> None:
        ...
        for idx, (sched, stage) in enumerate(zip(schedule, stage_map), 1):
            ...
            res = asyncio.run(self.copy_agent.process(product, {"persona": persona, "stage": stage}))
            email_camp = next((c for c in res["campaigns"] if c.get("channel") == "email"), None)
            if email_camp:
                print(f"  {C.BD}Asunto:{C.E} {email_camp.get('subject', 'N/A')}")
                for line in email_camp.get("body", "").split("\n"):
    ...
    def cmd_budget(self, budget_str: str = "10000", persona: str = "ecommerce_manager") -> None:
        ...
        uplift_signals = forest_result["channel_effects"]
        ...
        print(f"  {'─'*70}")
        for ch, data in res["allocation"].items():
        ...
        print(f"  {'─'*70}")
        print(f"  {C.G}{C.BD}Revenue Esperado: ${res['total_expected_revenue']:,.0f} | ROAS: {res['blended_roas']}x{C.E}\n")
    ...
    def cmd_evaluate(self, campaigns: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        scored = []
        for idx, (c, norm_v, r) in enumerate(zip(campaigns, norms, rewards), 1):
        ...
        winner = scored[0][0]
        print(f"\n{C.BD}{C.G}🏆 GANADOR: {winner.get('channel', '?').upper()} (Score: {scored[0][1]:.4f}){C.E}")
        return winner
    ...
    def cmd_causal(self, campaign: str, stage: str = "bofu", persona: str = "ceo_b2b") -> Dict[str, Any]:
        ...
        for ch, eff in result.get("channel_effects", {}).items():
            ...
            color = C.G if rec == "INVEST" else (C.Y if rec == "MAINTAIN" else C.R)
            ci = eff["confidence_interval"]
            print(f"  {ch:<18} {color}+{eff['uplift_pct']:>9.1f}%{C.E}   [{ci[0]:.2%}, {ci[1]:.2%}]   {eff['confidence']:>6.0%}   {color}{rec:>10}{C.E}")
    ...
    def cmd_dashboard(self) -> None:
        ...
        print(f"     • Caché: {len(self.cache.cache)}/{self.cache.max_size}")
        print(f"{C.H}{C.BD}{'='*72}{C.E}\n")
