    ...
    async def _process_message_async(self, message: StreamMessage):
        ...
        try:
            ...
            if message.message_type == MessageType.CONTROL:
                ...
                await self._handle_data_message_async(message)
...
def quick_streaming_setup() -> TruthGPTRealTimeManager:
    ...
    config = create_stream_config(
        host=os.environ.get("STREAMING_HOST", "localhost"),
        port=int(os.environ.get("STREAMING_PORT", 8080)),
        websocket_port=int(os.environ.get("STREAMING_WS_PORT", 8081)),
        sse_port=int(os.environ.get("STREAMING_SSE_PORT", 8082)),
        max_connections=100
    )
