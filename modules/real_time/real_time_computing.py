...
class StreamProcessor:
    ...
    async def _handle_text_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        ...
        # Process with real model if connected
        results = []
        for text in texts:
            try:
                # Real inference: We expect the user model to handle raw text or be pre-tokenized
                if hasattr(self.model, 'generate'):
                    output = self.model.generate(text)
                else:
                    raise NotImplementedError("Model lacks a valid 'generate' method for text processing.")
                results.append({"text": text[:50], "inference": str(output)})
            except Exception as e:
    ...
    async def _handle_audio_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        ...
        SOTA reference: Radford et al. (2022) "Robust Speech Recognition via
        Large-Scale Weak Supervision" (Whisper) — uses 80-bin log-Mel spectrograms
        at 16 kHz with 25 ms windows / 10 ms hop. We follow the same front-end
        contract so downstream acoustic models can be plugged in without code
        changes. If a model with an `encode_audio` or `transcribe` method is
        attached, we route raw waveforms to it; otherwise we extract robust
        spectral features and return structured telemetry (no simulation,
        no silent failures).
        """
        ...
        for ev in events:
            try:
                data = ev.data
                meta = ev.metadata or {}
                sample_rate = int(meta.get("sample_rate", sample_rate_default))
                # Coerce to tensor [num_samples] or [channels, num_samples]
                if isinstance(data, torch.Tensor):
                    wav = data.detach().to(torch.float32)
                elif isinstance(data, np.ndarray):
                    wav = torch.from_numpy(np.ascontiguousarray(data)).to(torch.float32)
                elif isinstance(data, (bytes, bytearray)):
                    wav = torch.frombuffer(bytes(data), dtype=torch.int16).to(torch.float32) / 32768.0
                elif isinstance(data, (list, tuple)):
                    wav = torch.tensor(data, dtype=torch.float32)
                else:
                    results.append({"event_id": ev.event_id, "error": f"unsupported audio payload type {type(data).__name__}"})
                    continue
                if wav.ndim > 1:
                    wav = wav.mean(dim=0)  # downmix to mono
                # Route to attached model if it exposes audio interface
                if self.model is not None and hasattr(self.model, "transcribe"):
                    out = self.model.transcribe(wav)
                    results.append({"event_id": ev.event_id, "transcription": str(out)})
                    continue
                if self.model is not None and hasattr(self.model, "encode_audio"):
                    emb = self.model.encode_audio(wav)
                    results.append({"event_id": ev.event_id, "embedding_shape": list(getattr(emb, "shape", []))})
                    continue
                # Front-end feature extraction (Whisper-compatible contract)
                ...
                # Robust scalar summary (avoid simulation; report real DSP stats)
                log_power = torch.log1p(power)
                results.append({
                    "event_id": ev.event_id,
                    "sample_rate": sample_rate,
                    "duration_s": float(wav.numel()) / float(sample_rate),
                    "rms": float(wav.pow(2).mean().sqrt().item()),
                    "spectral_centroid_hz": float(((torch.arange(power.shape[0]) * (sample_rate / n_fft)).unsqueeze(1) * power).sum() / (power.sum() + 1e-9)),
                    "log_power_mean": float(log_power.mean().item()),
                    "frames": int(power.shape[-1]),
                })
            except Exception as e:
                self.logger.error(f"audio handler failed for event {ev.event_id}: {e}")
                results.append({"event_id": ev.event_id, "error": str(e)})
        return {"stream_type": "audio", "events_processed": len(events), "results": results}

    async def _handle_video_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        """Handle video stream events.

        SOTA reference: Dosovitskiy et al. (2021) ViT and Radford et al. (2021)
        CLIP — frames are normalized to ImageNet statistics and resized to 224x224
        before being routed to a vision encoder. If the attached model exposes
        `encode_image` / `encode_video`, frames are forwarded directly; otherwise
        we compute deterministic per-frame statistics (mean, std, edge energy)
        useful for downstream anomaly detection.
        """
        results: List[Dict[str, Any]] = []
        ...
        for ev in events:
            try:
                data = ev.data
                if isinstance(data, torch.Tensor):
                    frames = data.detach().to(torch.float32)
                elif isinstance(data, np.ndarray):
                    frames = torch.from_numpy(np.ascontiguousarray(data)).to(torch.float32)
                else:
                    results.append({"event_id": ev.event_id, "error": f"unsupported video payload type {type(data).__name__}"})
                    continue
                # Expected shapes: [T,H,W,C] or [T,C,H,W] or [C,H,W]
                if frames.ndim == 3:
                    frames = frames.unsqueeze(0)
                if frames.shape[-1] in (1, 3) and frames.shape[1] not in (1, 3):
                    frames = frames.permute(0, 3, 1, 2)
                if frames.max() > 1.5:  # assume 0-255 input
                    frames = frames / 255.0
                ...
                if self.model is not None and hasattr(self.model, "encode_video"):
                    out = self.model.encode_video(normalized)
                    results.append({"event_id": ev.event_id, "embedding_shape": list(getattr(out, "shape", []))})
                    continue
                if self.model is not None and hasattr(self.model, "encode_image"):
                    out = self.model.encode_image(normalized)
                    results.append({"event_id": ev.event_id, "embedding_shape": list(getattr(out, "shape", []))})
                    continue
                # Deterministic structural stats (no simulation)
                gx = normalized[..., :, 1:] - normalized[..., :, :-1]
                gy = normalized[..., 1:, :] - normalized[..., :-1, :]
                edge_energy = float(gx.pow(2).mean().add(gy.pow(2).mean()).sqrt().item())
                results.append({
                    "event_id": ev.event_id,
                    "frames": int(frames.shape[0]),
                    "resolution": [int(frames.shape[-2]), int(frames.shape[-1])],
                    "mean_intensity": float(frames.mean().item()),
                    "std_intensity": float(frames.std().item()),
                    "edge_energy": edge_energy,
                })
            except Exception as e:
                self.logger.error(f"video handler failed for event {ev.event_id}: {e}")
                results.append({"event_id": ev.event_id, "error": str(e)})
        return {"stream_type": "video", "events_processed": len(events), "results": results}

    async def _handle_sensor_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        """Handle sensor stream events.

        SOTA reference: Zerveas et al. (2021) "A Transformer-based Framework
        for Multivariate Time Series Representation Learning" — sensor packets
        are normalized and summarized via sliding-window statistics (mean, std,
        min, max, slope). Real anomaly detection downstream can ingest the
        same dict contract. If the attached model exposes `encode_timeseries`,
        tensors are routed there directly.
        """
        results: List[Dict[str, Any]] = []
        for ev in events:
            try:
                ...
                if isinstance(payload, dict):
                    ...
                    channel = payload.get("channel", ev.metadata.get("channel", "unknown") if ev.metadata else "unknown")
                else:
                    values = payload
                    channel = ev.metadata.get("channel", "unknown") if ev.metadata else "unknown"
                if isinstance(values, (int, float)):
                    arr = torch.tensor([float(values)])
                elif isinstance(values, (list, tuple)):
                    arr = torch.tensor(values, dtype=torch.float32)
                elif isinstance(values, np.ndarray):
                    arr = torch.from_numpy(np.ascontiguousarray(values)).to(torch.float32)
                elif isinstance(values, torch.Tensor):
                    arr = values.detach().to(torch.float32).flatten()
                else:
                    results.append({"event_id": ev.event_id, "error": f"unsupported sensor payload type {type(values).__name__}"})
                    continue
                if arr.numel() == 0:
                    results.append({"event_id": ev.event_id, "error": "empty sensor payload"})
                    continue
                if self.model is not None and hasattr(self.model, "encode_timeseries"):
                    out = self.model.encode_timeseries(arr)
                    results.append({"event_id": ev.event_id, "channel": channel, "embedding_shape": list(getattr(out, "shape", []))})
                    continue
                # Robust statistics
                mean = float(arr.mean().item())
                std = float(arr.std(unbiased=False).item()) if arr.numel() > 1 else 0.0
                vmin = float(arr.min().item())
                vmax = float(arr.max().item())
                # Linear trend (least-squares slope)
                if arr.numel() > 1:
                    x = torch.arange(arr.numel(), dtype=torch.float32)
                    x_mean = x.mean(); y_mean = arr.mean()
                    num = ((x - x_mean) * (arr - y_mean)).sum()
                    den = ((x - x_mean) ** 2).sum().clamp_min(1e-9)
                    slope = float((num / den).item())
                else:
                    slope = 0.0
                # Z-score anomaly flag
                z = (arr - mean) / (std + 1e-9)
                anomalies = int((z.abs() > 3.0).sum().item())
                results.append({
                    "event_id": ev.event_id,
                    "channel": channel,
                    "samples": int(arr.numel()),
                    "mean": mean, "std": std, "min": vmin, "max": vmax,
                    "slope": slope, "anomaly_count": anomalies,
                })
            except Exception as e:
                self.logger.error(f"sensor handler failed for event {ev.event_id}: {e}")
                results.append({"event_id": ev.event_id, "error": str(e)})
        return {"stream_type": "sensor", "events_processed": len(events), "results": results}
    
    async def _handle_log_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        """Handle log stream events"""
        # Actual lightweight processing: count and validate
        valid_logs = [e for e in events if isinstance(e.data, str)]
        return {
            "stream_type": "log",
            "events_processed": len(valid_logs),
            "log_entries": len(valid_logs)
        }
    ...
    async def _handle_metric_stream(self, events: List[StreamEvent]) -> Dict[str, Any]:
        ...
        valid_metrics = [e for e in events if isinstance(e.data, (dict, float, int))]
        return {
            "stream_type": "metric",
            "events_processed": len(valid_metrics),
            "metrics_processed": len(valid_metrics)
        }
