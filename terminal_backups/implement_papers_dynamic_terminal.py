...
try:
    ...
    from latency_optimizations import (
        ...
        apply_dynamic_topology_routing, apply_atomic_memory,
    )
...
"""
    ...
    def apply_dynamic_topology_routing(query, agents, **kwargs): return {"avg_density": 1.0, "total_edges_saved": 0, "last_topology": {"num_active": len(agents) if agents else 0, "num_pruned": 0}}
    def apply_atomic_memory(observations, **kwargs): return {"op_counts": {"ADD": 0, "UPDATE": 0, "DELETE": 0, "NOOP": 0}, "memory_size": len(observations) if observations else 0, "compression_ratio": 0.0, "redundancy_dropped": 0}
...
DEFAULT_CONFIG = {
    ...
    "active_papers": ["chain_of_draft", "elastic_reasoning", "fp16_stability", "snap_kv", "speculative_decoding", "entropy_guided", "distinct_leaf", "discriminative_verification", "adaptive_kv_quant", "moqae", "confspec", "speculative_prefill", "intuitor", "echo", "reinforced_attention", "progressive_thought", "dytopo", "atommem"],
...
class PaperIntegrationEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.dytopo_active = "dytopo" in config.get("active_papers", [])
        self.atommem_active = "atommem" in config.get("active_papers", [])
        ...
        self.metrics = {
            ...
            "dytopo_edges_saved": 0,
            "dytopo_density": 1.0,
            "atommem_size": 0,
            "atommem_redundancy_dropped": 0,
        }
    ...
    def route_topology(self, query: str, agents: List[Dict[str, str]], rounds: int = 3) -> Dict[str, Any]:
        """Semantic-matching dynamic multi-agent topology routing (arXiv:2602.06039v1)."""
        if not self.dytopo_active or not agents:
            return {}
        result = apply_dynamic_topology_routing(query, agents, rounds=rounds)
        self.metrics["dytopo_edges_saved"] += result.get("total_edges_saved", 0)
        self.metrics["dytopo_density"] = result.get("avg_density", 1.0)
        return result

    def manage_memory(self, observations: List[str]) -> Dict[str, Any]:
        """Atomic-operation learnable agentic memory (arXiv:2601.08323v2)."""
        if not self.atommem_active or not observations:
            return {}
        result = apply_atomic_memory(observations)
        self.metrics["atommem_size"] = result.get("memory_size", 0)
        self.metrics["atommem_redundancy_dropped"] += result.get("redundancy_dropped", 0)
        return result
...
class DynamicPaperTerminal:
    ...
    def update_header(self):
        ...
        if self.paper_engine.dytopo_active:
            paper_names.append("DyTopo:ON")
        if self.paper_engine.atommem_active:
            paper_names.append("AtomMem:ON")
    ...
    def update_footer(self):
        ...
        metrics_table.add_row(
            f"[bold]DyTopo edges saved:[/bold] {m['dytopo_edges_saved']} (density {m['dytopo_density']:.2f})",
            f"[bold]AtomMem size:[/bold] {m['atommem_size']} | [bold]Redundancy dropped:[/bold] {m['atommem_redundancy_dropped']}"
        )
    ...
    async def run_continuous_loop(self):
        ...
        with self.live:
            ...
            while self.running and (self.cfg.get("max_iterations", 0) == 0 or self.iteration < self.cfg.get("max_iterations", 0)):
                ...
                # DyTopo dynamic multi-agent topology routing (arXiv:2602.06039v1)
                if self.paper_engine.dytopo_active:
                    agents = [
                    ...
                    dt = self.paper_engine.route_topology(f"plan and code a fix, then search evidence and verify correctness for iteration {self.iteration}", agents)
                    topo = dt.get("last_topology", {})
                    self.add_reasoning(f"   🕸️ DyTopo: {topo.get('num_active', 0)} active / {topo.get('num_pruned', 0)} pruned, {dt.get('total_edges_saved', 0)} edges saved")

                # AtomMem atomic-operation agentic memory (arXiv:2601.08323v2)
                if self.paper_engine.atommem_active:
                    observations = [
                        f"iteration {self.iteration} latency nominal",
                        f"iteration {self.iteration} latency nominal",  # duplicate -> NOOP
                        f"iteration {self.iteration} memory usage rising slightly",
                    ]
                    am = self.paper_engine.manage_memory(observations)
                    ops = am.get("op_counts", {})
                    self.add_workflow_log(f"   🧬 AtomMem: ops {ops} | size {am.get('memory_size', 0)} | dropped {am.get('redundancy_dropped', 0)}")
...
async def main():
    ...
    parser.add_argument("--no-dytopo", action="store_true", help="Disable DyTopo dynamic multi-agent topology routing")
    parser.add_argument("--no-atommem", action="store_true", help="Disable AtomMem atomic agentic memory")
