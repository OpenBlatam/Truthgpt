...
class TestModelRegistry(unittest.TestCase):
    ...
    def test_get_model_info(self):
        ...
        self.assertEqual(info["class"], "ModelManager")
        self.assertTrue(len(info["description"]) > 0)

        info_tgpt = get_model_info("truthgpt")
        self.assertEqual(info_tgpt["type"], "truthgpt")
        self.assertEqual(info_tgpt["class"], "TruthGPTModel")
    ...
    def test_custom_registration(self):
        ...
        self.assertIn("custom_dummy_model", list_available_models())
        instance = create_model("custom_dummy", {"test_key": 42})
        self.assertIsInstance(instance, CustomDummyModel)
        self.assertEqual(instance.config.get("test_key"), 42)
...
class TestTruthGPTModelArchitecture(unittest.TestCase):
    ...
    def test_config_initialization(self):
        ...
        self.assertEqual(cfg_dict["vocab_size"], 1000)
        self.assertEqual(cfg_dict["num_layers"], 2)
    ...
    def test_legacy_config_initialization(self):
        ...
        cfg_dict = config.to_dict()
        self.assertEqual(cfg_dict["vocab_size"], 500)
    ...
    def test_model_forward_pass(self):
        ...
        self.assertEqual(logits.shape, (batch_size, seq_len, 500))
        self.assertTrue(model.num_parameters() > 0)
    ...
    def test_model_size_reporting(self):
        ...
        self.assertIn("model_size_mb", size_info)
        self.assertTrue(size_info["total_parameters"] > 0)
...
class TestAttentionUtilities(unittest.TestCase):
    ...
    def test_positional_encoding(self):
        ...
        out = pe(x)
        self.assertEqual(out.shape, (32, 2, d_model))
    ...
    def test_attention_utils_alias(self):
        self.assertIs(AttentionUtils, AttentionOptimizer)


class TestModelManagerAndBuilder(unittest.TestCase):
    ...
    def test_model_manager_instantiation(self):
        ...
        self.assertTrue(hasattr(manager, "get_model_device"))
        self.assertTrue(hasattr(manager, "enable_multi_gpu"))
        self.assertTrue(hasattr(manager, "enable_torch_compile"))
        self.assertTrue(hasattr(manager, "configure_device_settings"))
    ...
    def test_model_builder_truthgpt(self):
        ...
        model = builder.build()
        self.assertIsInstance(model, TruthGPTModel)
        self.assertTrue(model.num_parameters() > 0)
...
class TestFactoryAndAliases(unittest.TestCase):
    ...
    def test_create_builder(self):
        ...
        self.assertIsInstance(bld, ModelBuilder)

    def test_create_diffusion(self):
        diff = create_model("diffusion")
        self.assertIsInstance(diff, DiffusionModelManager)

    def test_aliases_consistency(self):
        self.assertIs(DiffusionManager, DiffusionModelManager)
        self.assertIs(HFLLM, HFTransformersModel)
        self.assertIs(HFDiffusion, HFDiffusersModel)
        self.assertIs(build_model, create_model)
