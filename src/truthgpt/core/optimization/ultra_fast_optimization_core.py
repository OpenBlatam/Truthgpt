...
DEPRECATED: Use UnifiedOptimizer with OptimizationLevel.BASIC instead.
"""
import warnings
...
class UltraFastOptimizationCore(BaseOptimizationStrategy):
    """Backward-compatible facade around UnifiedOptimizer."""

    def __init__(self, config: Dict[str, Any] = None):
        warnings.warn(
            "UltraFastOptimizationCore is deprecated; "
            "use UnifiedOptimizer(level=OptimizationLevel.BASIC).",
            DeprecationWarning,
            stacklevel=2,
        )
        self.config = config or {}
        config_dict = self.config if isinstance(self.config, dict) else self.config.__dict__
        self._optimizer = UnifiedOptimizer(level=OptimizationLevel.BASIC, config=config_dict)
