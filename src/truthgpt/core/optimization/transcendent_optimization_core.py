...
DEPRECATED: Use UnifiedOptimizer with OptimizationLevel.LEGENDARY instead.
"""
import warnings
...
class TranscendentOptimizationCore(BaseOptimizationStrategy):
    """Backward-compatible facade around UnifiedOptimizer."""

    def __init__(self, config: Dict[str, Any] = None):
        warnings.warn(
            "TranscendentOptimizationCore is deprecated; "
            "use UnifiedOptimizer(level=OptimizationLevel.LEGENDARY).",
            DeprecationWarning,
            stacklevel=2,
        )
        self.config = config or {}
        config_dict = self.config if isinstance(self.config, dict) else self.config.__dict__
        self._optimizer = UnifiedOptimizer(level=OptimizationLevel.LEGENDARY, config=config_dict)
