...
def _invalidate_llm_client_cache() -> None:
    ...
    try:
        import truthgpt.interface.swarm_menu as swarm_menu
...
def log_event(layer: str, event: str, status: str = "DONE"):
    ...
    try:
        from truthgpt.interface.history_menu import _persist_event
    ...
    if theme in ["claude", "anthropic", "minimalist"]:
        from truthgpt.interface.cc_style import cc_log_event
...
def log_activity(module: str, task: str, status: str = "Completed"):
    ...
    theme = USER_PREFS.get("theme", "industrial")
    if theme in ["claude", "anthropic", "minimalist"]:
        from truthgpt.interface.cc_style import cc_log_activity
...
async def show_main_dashboard(extended: bool = False):
    theme = USER_PREFS.get("theme", "industrial")
    if theme in ["claude", "anthropic", "minimalist"]:
        from truthgpt.interface.cc_style import cc_divider, cc_action, cc_prompt_footer
        clear_screen()
        print(get_header())
        core_layers = [
...
def get_claude_header(updates: list[str] = None):
    ...
    right_content.append("\n █▓▒░ MISSION PERSISTENCE\n", style="white")
    from truthgpt.interface.core import background_missions
