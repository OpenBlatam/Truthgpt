...
class TransformerModel(nn.Module):
    ...
    def __init__(self, config: TransformerConfig):
        ...
        if config.use_embedding:
            self.embedding = nn.Embedding(config.vocab_size, truthgpt.config.d_model, padding_idx=config.pad_token_id)
        ...
        if config.use_output:
            self.output_layer = nn.Linear(config.d_model, truthgpt.config.vocab_size, bias=config.use_bias)
    ...
    def _create_positional_encoding(self, config: TransformerConfig):
        ...
        if config.positional_encoding == PositionalEncodingType.SINUSOIDAL:
            return SinusoidalPositionalEncoding(config.d_model, truthgpt.config.max_seq_length)
        elif config.positional_encoding == PositionalEncodingType.LEARNED:
            return LearnedPositionalEncoding(config.d_model, truthgpt.config.max_seq_length)
        elif config.positional_encoding == PositionalEncodingType.ROTARY:
            return RotaryPositionalEncoding(config.d_model, truthgpt.config.max_seq_length)
        elif config.positional_encoding == PositionalEncodingType.ALIBI:
            return AlibiPositionalEncoding(config.n_heads, truthgpt.config.max_seq_length)
        else:
            return None
...
if __name__ == "__main__":
    ...
    input_ids = torch.randint(0, truthgpt.config.vocab_size, (batch_size, seq_length))
