...
class ReasoningModel(nn.Module):
    ...
    def __init__(self, config: ReasoningExpertConfig):
        ...
        # Input embedding
        self.input_embedding = nn.Linear(config.hidden_size, truthgpt.config.hidden_size)
        ...
        # Output projection
        self.output_projection = nn.Linear(config.hidden_size, truthgpt.config.hidden_size)
...
class ReasoningLayer(nn.Module):
    ...
    def __init__(self, config: ReasoningExpertConfig, layer_idx: int):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(config.hidden_size, truthgpt.config.intermediate_size),
            nn.GELU(),
            nn.Dropout(config.dropout),
            nn.Linear(config.intermediate_size, truthgpt.config.hidden_size)
        )
