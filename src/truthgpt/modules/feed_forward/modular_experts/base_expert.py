...
class ExpertHiddenLayer(nn.Module):
    ...
    def __init__(self, config: ExpertConfig, layer_idx: int):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(config.hidden_size, truthgpt.config.intermediate_size, bias=config.use_bias),
            self._get_activation(config.activation),
            nn.Dropout(config.dropout),
            nn.Linear(config.intermediate_size, truthgpt.config.hidden_size, bias=config.use_bias)
        )
