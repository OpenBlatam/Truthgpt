...
class TextEncoder(ModalityEncoder):
    ...
    def __init__(self, config: MultimodalConfig, vocab_size: int = 30000):
        ...
        self.embedding = nn.Embedding(vocab_size, truthgpt.config.hidden_dim)
        self.positional_encoding = nn.Parameter(torch.randn(config.max_sequence_length, truthgpt.config.hidden_dim))
...
class ImageEncoder(ModalityEncoder):
    ...
    def __init__(self, config: MultimodalConfig):
        ...
        self.projection = nn.Linear(256 * 7 * 7, truthgpt.config.hidden_dim)
...
class AudioEncoder(ModalityEncoder):
    ...
    def __init__(self, config: MultimodalConfig):
        ...
        self.projection = nn.Linear(256, truthgpt.config.hidden_dim)
...
class AdaptiveFusion(nn.Module):
    ...
    def __init__(self, config: MultimodalConfig):
        ...
        self.modality_projections = nn.ModuleList([
            nn.Linear(config.hidden_dim, truthgpt.config.fusion_dim) 
            for _ in range(self.num_modalities)
        ])
        
        # Fusion network
        self.fusion_network = nn.Sequential(
            nn.Linear(config.fusion_dim * self.num_modalities, truthgpt.config.hidden_dim),
            nn.ReLU(),
            nn.Dropout(config.dropout_rate),
            nn.Linear(config.hidden_dim, truthgpt.config.hidden_dim)
        )
...
class MultimodalFusionEngine(nn.Module):
    ...
    def __init__(self, config: MultimodalConfig):
        ...
        # Output projection
        self.output_projection = nn.Linear(config.hidden_dim, truthgpt.config.fusion_dim)
