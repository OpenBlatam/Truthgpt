...
from optimization_core.agents.framework.exceptions import RoutingError
from optimization_core.agents.domains.intelligence.cost_intelligence import get_cost_intelligence

try:
    from interface.cc_style import cc_action, cc_result
    CC_AVAILABLE = True
except ImportError:
    CC_AVAILABLE = False
...
class RouterDecision(BaseModel):
    ...
    reasoning: str = Field(
        ...
        validation_alias=AliasChoices("reasoning", "thought")
    )
    target_agent: Optional[str] = Field(
        None, 
        description="Exact registered name of the selected agent",
        validation_alias=AliasChoices("target_agent", "handoff")
    )
...
class SwarmOrchestrator:
    ...
    async def route_and_process(
        ...
        target_dir = None
        try:
            from interface.core import extract_target_directory
            target_dir = extract_target_directory(query)
        except Exception:
            pass
            
        if target_dir and not context.get("_dir_injected"):
            context["_dir_injected"] = True
            try:
                ...
                if path_obj.exists() and path_obj.is_dir():
                    for entry in path_obj.iterdir():
                        if entry.is_file():
                            files_info.append(f"- {entry.name} ({entry.stat().st_size} bytes)")
                        elif entry.is_dir() and not entry.name.startswith('.'):
                            files_info.append(f"- {entry.name}/ (directory)")
                    
                    if files_info:
                        files_str = "\n".join(files_info[:30])
                        if len(files_info) > 30:
                            files_str += f"\n- ... and {len(files_info) - 30} more files"
                    else:
                        files_str = "(empty directory)"
                        
                    dir_context = (
                        f"### TARGET DIRECTORY DETECTED\n"
                        f"You are working in the target directory: {target_dir}\n"
                        f"Existing files in this directory:\n"
                        f"{files_str}\n\n"
                        f"If you write any code blocks, please place a comment on the first line indicating the path where it should be saved relative to this target directory. "
                        f"Example: `# app.py` or `# utils/helpers.py`. Do NOT write output filenames anywhere else.\n"
                        f"Now process the request.\n\n"
                    )
                    query = dir_context + query
            except Exception as e:
                logger.warning(f"Could not list directory {target_dir}: {e}")
        ...
        if CC_AVAILABLE:
            cc_action(f"Routing to expert: {target_agent.name}", status="RUN")
        logger.info("Routing query to agent: %s", target_agent.name)
        
        # Using the standard Agent processing loop
        response = await target_agent.process(query, context=context)
    ...
    async def _route_via_llm(self, query: str) -> Optional[BaseAgent]:
        ...
        prompt = (
            "### MASTER ROUTER ROLE\n"
            "You are the Master Router of a SOTA Multi-Agent Swarm.\n"
        ...
        for attempt in range(self.swarm_config.max_routing_retries):
            try:
                # Optimized call via Cost Intelligence
                decision_raw = await get_cost_intelligence().optimize_call(
                    prompt=prompt, 
                    llm_func=self.llm
                )
