...
class AgentClient:
    ...
    def _init_default_swarm(self) -> None:
        ...
        # In System 5.9, we don't pre-register; the ComponentRegistry already has the lazy map.
        # We only instantiate the 'system_agent' as a primary responder if it's already in the registry.
    ...
    def _register_default_tools(self) -> None:
        ...
        # Iterate over the lazy tool map to avoid pre-loading classes
        for tool_name in list(registry._tool_map.keys()):
            try:
                # Still check built-in registry names
                tool_cls = registry.get_tool(tool_name)
                if not tool_cls: continue
    ...
    async def resume_task(self, task_id: str) -> Union[str, AgentResponse]:
        ...
        if self.agent is None:
            raise RuntimeError("Resumption only supported in single-agent mode for now.")
