...
class CoreMemory:
    """
    Persistent Working Memory for agents with thread-safe SQLite access.
    ...
    def _get_core_sync(self, user_id: str) -> Dict[str, str]:
        """Synchronous helper for get_core with thread lock."""
        with self._lock:
            try:
                with sqlite3.connect(str(self.db_path)) as conn:
                    ...
                    if row:
                        return {"persona": row["persona"] or "", "human": row["human"] or ""}
    ...
    def _update_block_sync(self, user_id: str, block: str, content: str) -> bool:
        """Synchronous helper for update_block with thread lock."""
