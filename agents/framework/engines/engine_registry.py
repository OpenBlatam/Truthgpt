...
from optimization_core.agents.orchestration.ensemble.ensemble import ALL_ENSEMBLE_MODES, run_ensemble


def _get_user_prefs():
    return engine_config._get_user_prefs()


def _load_api_keys_from_prefs():
    return engine_config._load_api_keys_from_prefs()


def _normalize_engine_key(name: str) -> str:
    return engine_config._normalize_engine_key(name)
...
class EngineRegistry:
    ...
    def _find_fallback_provider(self, target_name: str, fallback_order: List[str]) -> tuple[Optional[BaseProvider], str]:
        for f_name in fallback_order:
            ...
            if f_provider and f_provider.api_key:
                return f_provider, norm
        return None, target_name

    def _parse_and_register_custom_model(self, name: str) -> Optional[BaseProvider]:
        if ":" not in name:
            return None
    ...
    def _resolve_provider(self, name: Optional[str]) -> tuple[Optional[BaseProvider], str]:
        ...
        if not resolved_name or base_provider not in self._default_providers:
            f_provider, norm_name = self._find_fallback_provider("", fallback_order)
            if norm_name:
                resolved_name = norm_name
        ...
        if not provider and name and ":" in name:
            provider = self._parse_and_register_custom_model(name)

        if provider and not provider.api_key:
            f_provider, norm_name = self._find_fallback_provider(resolved_name, fallback_order)
            if f_provider:
                logger.warning(f"Preferred engine '{resolved_name}' lacks API key. Falling back to '{norm_name}'")
                provider = f_provider
                resolved_name = norm_name
    ...
    def build_tiered_engine(
        ...
        for raw in str(preferred_raw).split(","):
            key = _normalize_engine_key(raw.strip())
        ...
        if cost_optimized:
            engine = self._build_cost_optimized_engine(tier, base_keys)
    ...
    def _build_cost_optimized_engine(
        self, tier: str, base_keys: List[str]
    ) -> Optional[AsyncLLMEngine]:
